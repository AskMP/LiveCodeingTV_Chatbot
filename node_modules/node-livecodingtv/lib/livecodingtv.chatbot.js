...
(function () {
    ...
    *   roll(string who) = string of the viewer’s roll ["owner"|"moderator"|"viewer"]
    ...
    *   on('online')
    *   on('offline')
    *   on('join', { who: STRING, roll: ["owner"|"moderator"|"viewer"]})
    *   on('leave', STRING)
    *   on('message', { from: STRING, roll: ["owner"|"moderator"|"viewer"], message: STRING, stanza: OBJECT })
    *
    ******************************************************/
    var ChatBot = function (supplied_config) {
        ...
        var self = this, LiveCodingTV,
            ...
            // The config object above can contain all the credentials if needed to or there can be a
            // supplied object during creation
            init = function (supplied_config) {
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    if (data.message.indexOf('!') === 0) {
                        if (data.message.indexOf(':') === -1) {
                            cmd = data.message = data.message.substring(1);
                        } else {
                            cmd = data.message.substring(1, data.message.indexOf(':'));
                        }
                    }
            ...
            receiveMessage = function (stanza) {
                ...
                if (!stanza.getChild('delay') && from !== config.jid) {
                    applyCommands({
                        from: from,
                        roll: self.roll(from),
                        message: message,
            ...
            // Capture any new presence as a what their roll is. Anybody who leaves, simply
                ...
                if (typeof stanza.attrs.type !== 'undefined' && stanza.attrs.type === 'unavailable') {
                    ...
                    case "owner":
                        ...
                        if (!stanza.getChild('delay')) {
                            self.emit('join', { who: who, roll: 'owner'});
                        }
                        break;
                    case "admin":
                        if (channel.mods.indexOf(who) === -1) {
                            ...
                            if (!stanza.getChild('delay')) {
                                self.emit('join', { who: who, roll: 'moderator'});
                            }
                        }
                        break;
                    default:
                        if (channel.viewers.indexOf(who) === -1) {
                            channel.viewers.push(who);
                            if (!stanza.getChild('delay')) {
                                self.emit('join', { who: who, roll: 'viewer'});
                            }
                        }
                    }
                }
        ...
        // Capture the roll of a specific user
        this.roll = function (who) {
